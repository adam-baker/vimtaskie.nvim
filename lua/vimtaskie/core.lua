...
function M.open_comment_editor(task_id)
  ...
  vim.api.nvim_buf_set_keymap(buf, "n", "ZZ", "", {
    noremap = true,
    silent = true,
    callback = function() M.save_comment(task_id, buf) end,
    desc = "Save comment",
  })
...
function M.save_comment(task_id, buf)
  ...
  local query = string.format("INSERT INTO comments (task_id, comment, timestamp) VALUES (%d, '%s', '%s')", task_id, comment_text, timestamp)
  local res, err = conn:execute(query)
  if not res then
